...
--can't we use otherwise, or something of that sort?
zip _ _ = []
...
myReverse Nil = Nil
myReverse (Cons a xs)  = myReverse xs +++ Cons a Nil
...
treeSum Empty = 0
treeSum (Node a st0 st1) = a + (treeSum st0) + (treeSum st1) 
...
treeHeight Empty = 0
treeHeight (Node a st0 st1) = max (treeHeight st0) (treeHeight st1) + 1 
